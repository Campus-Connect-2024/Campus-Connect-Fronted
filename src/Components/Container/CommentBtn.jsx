...
const CommentBtn = ({svg, className, postId}) => {
  ...
  return ( 
    <div className=' pl-5 py-2 cursor-pointer flex justify-center items-center'>              
        <button className='flex gap-3 text-black cursor-pointer items-center' ><img src={svg} className={` ${className}`} alt="like" /> {comments.length} </button>
    </div>
  )
}
